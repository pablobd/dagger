...
class NotEqual:
    ...
    def __init__(
        ...
        """
        Initialize a NotEqual when clause.
        ...
        value
            The value we're expecting the parameter to not be equal to, once deserialized
    ...
    def evaluate_condition(self, params: Dict[str, Any]) -> bool:
        """
        Return true if the parameter is not equal to the specified value.
        ...
        if self._param_name not in params:
            raise ValueError(
                f"Parameter '{self._param_name}' was expected to not be equal to '{self._value}' (type: {type(self._value).__name__}). However, no parameter with that name was passed to this node. The node received the following parameters {list(params)}"
            )
