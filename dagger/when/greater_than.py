"""Compare the value of a node input with a value and expect the input to be greater than the value."""
...
class GreaterThan:
    ...
    def __init__(
        ...
        """
        Initialize a GreaterThan when clause.
        ...
        value
            The value we're expecting the parameter to be greater then, once deserialized
    ...
    def evaluate_condition(self, params: Dict[str, Any]) -> bool:
        """
        Return true if the parameter is greater than the specified value.
        ...
        if self._param_name not in params:
            raise ValueError(
                f"Parameter '{self._param_name}' was expected to be greater than '{self._value}' (type: {type(self._value).__name__}). However, no parameter with that name was passed to this node. The node received the following parameters {list(params)}"
            )
